...
bool compareScore(const TextBox &a, const TextBox &b) {
...
float boxScoreFast(const cv::Mat &inMat, const std::vector<cv::Point> &inBox) {
    ...
    auto m =  cv::mean(inMat(cv::Rect(cv::Point(minX, minY), cv::Point(maxX + 1, maxY + 1))).clone(),
                    maskMat);
    return m.val[0];
}
...
std::vector<cv::Mat> getPartImages(const cv::Mat &src, std::vector<TextBox> &textBoxes) {
    std::sort(textBoxes.begin(), textBoxes.end(), compareScore);
